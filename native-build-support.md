...
The compiled binary is written to `target/cmd-rest`.
...
The real list has grown well past this excerpt — see `CmdRestApplication` for the
authoritative set:
...
@RegisterReflectionForBinding({
  ...
  TestRunResult.class, TestCaseResult.class, AssertionFailure.class,
  // ... see the class for the full list
})
...
> **Superseded by section 9.** `SoftAssertions` is no longer used or registered — it was
> replaced by `FailureCollector`. This subsection records why registering it was tried
> first and why registration alone could never have worked.

`SoftAssertions.assertSoftly()` instantiates `SoftAssertions` via `Class.getConstructor()`.
Without reflection registration this fails at runtime with `NoSuchMethodException`, so it
was added to the list above — which turned out not to be enough. See section 9.
...
**`src/main/resources/META-INF/native-image/io.github.snytkine.cmdrest/cmd-rest/reflect-config.json`**

```json
[
  {
    "name": "io.github.snytkine.cmdrest.CmdRestApplication__ApplicationContextInitializer",
...
are useful during development (e.g. `logging.level.io.github.snytkine.cmdrest=DEBUG`) are
...
```properties
logging.level.io.github.snytkine.cmdrest=OFF
```
