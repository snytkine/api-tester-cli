...
- **Documentation sync** — after any change that affects how an end-user invokes a command, alters command-line options or their behaviour, adds or removes environment variables, or introduces new configuration options, update **both** `README.md` here and the relevant file(s) in the separate `cmdrest-web` project at `../cmdrest-web/src/docs/`. The files most likely to need updating are `cli-reference.md`, `getting-started.md`, `environment-variables.md`, and `test-suite-configuration.md`. The `docs/` folder no longer exists in this repo — feature documentation lives in `cmdrest-web`. Documentation must always reflect the current behaviour.
