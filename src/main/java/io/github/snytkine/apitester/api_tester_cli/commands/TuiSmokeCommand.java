 ...
 * the caller thread (which updates content) and the render thread (which reads it via the
 * {@code drawFunction}).
 */
@Component
public class TuiSmokeCommand {
   ...
   *
   * <p>This command is intentionally kept simple: all state lives in a single
   * {@link AtomicReferenceArray} that is shared between this thread and the render thread.
   *
  ...
  @Command(
      name = "tui-smoke",
      description =
          "Smoke test: verifies that GridView + BoxView render and update correctly."
  ...
  private void animateRow(
      ViewComponent vc, AtomicReferenceArray<String> rows, int testIdx)
