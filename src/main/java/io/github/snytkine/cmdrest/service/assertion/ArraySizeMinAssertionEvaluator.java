 ...
 * that it is a JSON array with at least {@code min} elements using an AssertJ size assertion.
 *
    ...
    public void evaluate(ApiResponse response, FailureCollector collector) {
        switch (ResponseValueExtractor.extract(response, assertion.path())) {
            case Result.Found f -> {
                ...
                try {
                    Assertions.assertThat(list).hasSizeGreaterThanOrEqualTo(assertion.min());
