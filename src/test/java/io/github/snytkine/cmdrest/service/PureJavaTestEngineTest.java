...
class PureJavaTestEngineTest {
    ...
    void passedTestResultHasRequestInfoPopulated() throws Exception {
        ...
        assertThat(firstResult.requestInfo().method()).isEqualTo(io.github.snytkine.cmdrest.model.HttpMethod.GET);
    ...
    void skippedTestFiresSkipEventToListener() throws Exception {
        ...
        var events = new java.util.ArrayList<io.github.snytkine.cmdrest.event.TestProgressEvent>();
        engine.runConfigurationSuite(suite, SuiteRunContext.of(Map.of(), Map.of()), events::add);

        long skipEvents = events.stream()
                .filter(e -> e instanceof io.github.snytkine.cmdrest.event.TestProgressEvent.TestCompleted tc
                        && tc.status() == io.github.snytkine.cmdrest.event.TestStatus.SKIP)
                .count();
